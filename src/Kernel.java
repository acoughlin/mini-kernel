...
//"/".
    ...
    static Hashtable<String, FileObject> myFiles = new Hashtable<String, FileObject>();
    private static int doFormat(){
        disk = new Disk(disk.DISK_SIZE);
        myFiles.clear();
        return 0;
    }
//Creates a new file with the indicated filename. The initial contents of the file are all
//null (zero) bytes.
    private static int doCreate(String filename){
        disk.currentBlock++;
        FileObject newFile = new FileObject();
        newFile.fileBlock = disk.currentBlock;
        newFile.fileSize = 0;   // in bits
        myFiles.put(filename, newFile);
        Utilities.pack(filename, disk.data, newFile.fileBlock*disk.data.length/disk.DISK_SIZE);
        return 0;
    }
//Reads the contents of the file indicated by filename into buffer
    private static int doRead(String filename){
        disk.currentBlock = myFiles.get(filename).fileBlock;
        int endData = Utilities.unpackString(disk.data, disk.currentBlock*disk.data.length/disk.DISK_SIZE).indexOf("");
        doOutput(Utilities.unpackString(disk.data, disk.currentBlock*disk.data.length/disk.DISK_SIZE).substring(0, myFiles.get(filename).fileSize)+"\n");
        return 0;
    }

    private static int doWrite(String fileName, String data){
        disk.currentBlock=myFiles.get(fileName).fileBlock;
        myFiles.get(fileName).fileSize=data.getBytes().length;
        Utilities.pack(data, disk.data, disk.currentBlock*disk.data.length/disk.DISK_SIZE);      
        return 0;
    }
//Overwrites the file indicated by filename with data from buffer.
    private static int doDelete(String filename){
        int p = 0;
        while (p < myFiles.get(filename).fileSize){
            disk.data[myFiles.get(filename).fileBlock*disk.data.length/disk.DISK_SIZE+p]=0;
            p++;
        }
        myFiles.remove(filename);
        return 0;
    }
//Destroys the indicated file. It is an error if the indicated file does not exist.
    private static int doDir(String [] t){
        t[0] = System.getProperty("user.dir");
        for(String e : myFiles.keySet())
            doOutput(e+"\n");
        return 0;
    }
//Displays the contents of the directory on System.out.
    ...
    private static int doGetLocal(String[] t) {
        t[0] = System.getenv("COMPUTERNAME");
        return 0;
    }
...
//amc code
                ...
                case SYS_LOCAL:
                    return doGetLocal((String[]) o1);
                
                case SYS_DIR:
                    return doDir((String[]) o1);
                ...
                case SYS_READ:
                    return doRead((String) o1);
                case SYS_WRITE:
                    //Kernel.SYS_WRITE, offset, fname, buf, null);
