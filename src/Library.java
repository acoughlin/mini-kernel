...
//amc code
    ...
    public static String getLocal() {
        ...
        int rc = Kernel.interrupt(Kernel.INTERRUPT_USER,
                Kernel.SYS_LOCAL,
                0, t, null, null);
// if SYS_LOCAL returned error then return error
        if (rc < 0) {
            return String.valueOf(rc);
        } // else return output of SYS_LOCAL
        else {
            return t[0];
        }
    }
...
// if SYS_LOCAL returned error then return error
    ...
    public static int format(){ 
        return Kernel.interrupt(Kernel.INTERRUPT_USER,
            Kernel.SYS_FORMAT, 0, null, null, null);
    }// format
    
    public static int create(String fileName){ 
        return Kernel.interrupt(Kernel.INTERRUPT_USER,
            Kernel.SYS_CREATE, 0, fileName, null, null);
    }// create
    
    public static int read(String fname){ 
        return Kernel.interrupt(Kernel.INTERRUPT_USER,
            Kernel.SYS_READ, 0, fname, null, null);
    }// read
    
    public static int write(String fname, String data){ 
        return Kernel.interrupt(Kernel.INTERRUPT_USER,
            Kernel.SYS_WRITE, 0, fname, data, null);
    }// write
